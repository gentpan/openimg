...
- **双主题**：深色 / 浅色，切换只换一组 CSS 变量，不触发任何重渲染
...
**对象 key 是 `2026/08/05/aB3dEf7hJ9kL.jpg`。** 日期取 UTC 上传日，ID 为 12 位 `0-9a-zA-Z`（拒绝采样避免模偏）。去重不依赖 key 的算法——命中 `(profile_id, sha256)` 后复用已有 key，所以 key 格式可以改。变体路径由已存的 key 推导而非从 sha 重算，因此历史上的内容寻址布局无需迁移即可共存。

**账本是权威，计数器是缓存。** `quota_transactions` 可重放校验，`QuotaBytes` / `UsedBytes` 只是冗余计数器，启动时自动回填缺失记录。

**删除走引用计数。** 去重让多个用户共享同一对象，purge 任务确认无其他活跃引用才真正删除。
