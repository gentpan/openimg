...
var zh = map[string]string{
	...
	"ai.purpose_unknown":          "不支持的生成用途",
	"ai.resolution_unknown":       "不支持的画质档位",
	"ai.resolution_denied":        "4K 画质需要先关联 pic.bi 账号",
	"ai.resolution_needs_session": "这个清晰度需要在网页登录后使用，API 令牌只能用免费额度",
	// 与 ai.no_credits 分开:那句是"你没钱了，去签到"，这句是"我们此刻问不到
	// 你的余额"。混成一句，用户会按前者去签到，然后发现签了也没用。
	"ai.picbi_unreachable": "暂时联系不上 pic.bi，请稍后再试",
	"ai.picbi_denied":      "pic.bi 拒绝了这次扣费，请到 pic.bi 检查授权与账号状态",
