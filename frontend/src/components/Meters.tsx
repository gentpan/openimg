        ...
        {hover != null && (
          <>
            <line
              ...
              opacity={0.5}
            />
            <circle cx={pts[hover].x} cy={pts[hover].y} r={4} fill="var(--color-violet-400)" vectorEffect="non-scaling-stroke" />
          </>
        )}
      </svg>
      ...
      {active && (
        <div className="absolute top-0 left-0 right-0 flex justify-center pointer-events-none">
          <div className="rounded-lg bg-neutral-950/90 border border-neutral-800 px-2 py-1 text-[10px] text-neutral-300 whitespace-nowrap">
            {active.label} · <span className="text-violet-300">{formatValue(active.value)}</span>
          </div>
        </div>
      )}
    </div>
