...
export const en: Dict = {
  ...
  generate: {
    ...
    quota: {
      inline: {
        times: (n: number) => `${n} left`,
        today: (used: number, limit: number) => `Today ${used}/${limit}`,
        monthly: (left: number, total: number) => `Monthly ${left}/${total}`,
        granted: (n: number) => `Bonus ${n}`,
        expiring: (n: number, date: string) => `${n} expire ${date}`,
        picbi: (n: number) => `pic.bi ${n}`,
      },
      ...
      monthlyExhaustedHint: "Check in daily for +1; 10/20/30-day streaks add 3/5/10",
  ...
  retouch: {
    ...
    presets: {
      ...
      expand: "Extend canvas",
    },
    presetClear: "Click again to clear",
